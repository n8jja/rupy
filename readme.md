...
## What does it do?
The libray contains 2 files:
- gps.py
- wl2k.py

The goal is to allow for easy interfacing of a Windows PC with email-over-radio and provide a means
of updating station information when off-grid or otherwise in an unconnected environment.  There is an excellent library
for handling GPS functions called gpsd, however it is available for Linux and Mac only.  As a result, functionality had to be 
developed from scratch.  This is not just a clone of that library, but rather a broader collection of radio-specific utilities.
...
NMEA 0183 format.  Some devices, such as the inexpensive Ublox-based units, will **NOT** work at this time, although support\
is planned.

#### GPS Class:
Generalized class for all GPS units.  Requires a COM port, baudrate and timeout to instantiate.  Only the COM port is requierd
since most units utilize the default baud and timeout of 9600 and 3 respectively.
...
#### Usage:
```
    # Create a new instance of a GPS units
    gps = Gps(COM1) # <= Looks for a GPS on COM1 with baud=9600 and timeout=3
    gps2 = Gps(COM1, 1200, 5) # <= overrides the default baud and timeout values.
...
### wl2k.py
This file contains functions specifically for interacting with the Winlink Express web API and the Winlink application config file
